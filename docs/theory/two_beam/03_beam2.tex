...
\citet{gutjahr2011} provides a quantitative model in which current work allocation changes
future competence.
...
\begin{equation}
C_{t+1}=\mathcal{L}(C_t,x_t;\theta_t),
\label{eq:learning}
\end{equation}
where \(x_t\) is operational experience and \(\theta_t\) contains learning and retention
parameters. Equation~\eqref{eq:learning} is a synthesis, not a formula asserted by the cited
literature.
